...
void ace_synth_default_params(AceSynthParams * p) {
    ...
    p->adapter_scale_cross = 1.0f;
    p->adapter_scale_mlp   = 1.0f;
    p->use_fa            = true;
    p->clamp_fp16        = false;
...
AceSynth * ace_synth_load(ModelStore * store, const AceSynthParams * params) {
    ...
    ctx->dit_key.adapter_scale_self  = params->adapter_scale_self;
    ctx->dit_key.adapter_scale_cross = params->adapter_scale_cross;
    ctx->dit_key.adapter_scale_mlp   = params->adapter_scale_mlp;
    ...
    if (params->adapter_path) {
        fprintf(stderr, "[Synth-Load] Adapter: %s (scale=%.2f self=%.2f cross=%.2f mlp=%.2f)\n",
                params->adapter_path, params->adapter_scale, params->adapter_scale_self, params->adapter_scale_cross,
                params->adapter_scale_mlp);
    }
